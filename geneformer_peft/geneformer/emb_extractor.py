...
def get_embs(model,
    ...
    for i in trange(0, total_batch_length, forward_batch_size):
        ...
        input_data_minibatch = pad_tensor_list(input_data_minibatch,
                                               ...
                                               pad_token_id,
                                               model_input_size)
        attention_mask = (input_data_minibatch != pad_token_id).int()
...
def plot_umap(embs_df, emb_dims, label, output_file, kwargs_dict):
    ...
    sc.tl.umap(adata)
    sil_score = silhouette_score(only_embs_df, adata.obs[label])
    print(f'Silhouette Score: {sil_score:.4f}')

    #plt.text(plt.xlim()[1], plt.ylim()[0], f'Silhouette Score: {sil_score:.4f}', ha='right', va='bottom', fontsize=8)
