...
class TranscriptomeTokenizer:
    ...
    def tokenize_data(
        ...
        """
        tokenized_cells, cell_metadata = self.tokenize_files(a,b,adata_scale,
    ...
    def tokenize_files(
            self, a,b,adata_scale, data_directory, file_format: Literal["loom", "h5ad"] = "loom"
    ):
        tokenized_cells = []
        if self.custom_attr_name_dict is not None:
            loom_cell_attr = [attr_key for attr_key in self.custom_attr_name_dict.keys()]
        ...
        for file_path in data_directory.glob("*.{}".format(file_format)):
            ...
            file_tokenized_cells, file_cell_metadata = tokenize_file_fn(a,b,adata_scale, file_path)
    ...
    def tokenize_anndata(self,a,b, adata_scale, adata_file_path):

        adata_all = ad.read(adata_file_path)
        self.hvg_flavor = "cell_ranger"  #if self.data_is_raw else "cell_ranger" , "seurat_v3"
        adata_train=adata_all[a]
        ...
        adata_test = adata_all[b]
        index_2000=adata_test.var_names.get_indexer(adata_train.var_names)
        adata_test = adata_test[:, index_2000]
        for adata in [adata_train,adata_test] :
            ...
            X_norm = (adata_filter.X / adata.X.sum(1) * 10_000 / norm_factor_vector)
