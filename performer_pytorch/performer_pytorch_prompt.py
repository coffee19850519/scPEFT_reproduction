...
class SelfAttention(nn.Module):
    def __init__(
        ...
        if self.prompt_type == 'LoRA':
            self.to_q = lora.Linear(dim, inner_dim, r=8)
            self.to_k = nn.Linear(dim, inner_dim)
            self.to_v = lora.Linear(dim, inner_dim, r=8)
        else:
...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len):
        super().__init__()
        gene2vec_weight = np.load('/fs/ess/PAS1475/Yang/yy/scBERT_pretrain_data/gene2vec_16906.npy')
...
class Performer(nn.Module):
    def __init__(
        ...
        self.prompt_type = prompt_type
        self.space_conf = space_conf
        self.mlp_conf = mlp_conf
        self.Encoder_Space = nn.ModuleList([])
        self.Encoder_MLP = nn.ModuleList([])
        if self.prompt_type == 'Gene_encoder_prompt':
            for i in range(depth):
                self.Encoder_Space.append(
                    wrapper_fn(Adapter(dim)) if self.space_conf[i] else nn.Identity())
            for i in range(depth):
                self.Encoder_MLP.append(
                    wrapper_fn(Adapter(dim, skip_connect=False)) if self.mlp_conf[i] else nn.Identity())


        for _, local_heads in zip(range(depth), local_attn_heads):
            layers.append(nn.ModuleList([
                wrapper_fn(SelfAttention(dim, causal = causal, heads = heads, dim_head = dim_head, local_heads = local_heads, local_window_size = local_window_size, nb_features = nb_features, generalized_attention = generalized_attention, kernel_fn = kernel_fn, dropout = attn_dropout, no_projection = no_projection, qkv_bias = qkv_bias,prompt_type=self.prompt_type)),
        ...
        self.net = execute_type(layers, args_route = {**attn_route_map, **context_route_map},
                                prompt_type=self.prompt_type,Encoder_Space=self.Encoder_Space,Encoder_MLP=self.Encoder_MLP,dim=dim)
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.space_conf = space_conf
        self.mlp_conf = mlp_conf
        if self.prompt_type == 'Gene_token_prompt':
            self.Space_Adapter = Adapter(dim,mlp_ratio)
        if g2v_position_emb:
        ...
        self.performer = Performer(
                                   ...
                                   space_conf=self.space_conf,
                                   mlp_conf=self.mlp_conf,
                                   )
        self.norm = nn.LayerNorm(dim)
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        # print(x.shape)
        # gene mask
        # new_gene_mask = [True] + self.gene_mask
        # x = x[:,new_gene_mask,:]
