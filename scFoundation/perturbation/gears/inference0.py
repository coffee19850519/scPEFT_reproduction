...
def evaluate(loader, model, uncertainty, device):
    ...
    for itr, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for itr, de_idx in enumerate(batch.de_idx):
                ...
                pert_cat_de.append(batch.pert[itr])
   ...
   #-----
    if len(pred_de) > 0:
        ...
        pred_de = torch.empty((0,), dtype=torch.float32)
        truth_de = torch.empty((0,), dtype=torch.float32)
    # -----

    results['pred_de']= pred_de.detach().cpu().numpy()
    results['truth_de']= truth_de.detach().cpu().numpy()
    results['pert_cat_de'] = np.array(pert_cat_de)
...
def compute_metrics(results, gene_idx=None):
    ...
    for pert in np.unique(results['pert_cat']):
        ...
        if pert != 'ctrl':
            ...
            # -----
            # print("Checking", pert)
            # print("DE pred shape:", results['pred_de'][p_idx_de].shape)
            # print("DE truth shape:", results['truth_de'][p_idx_de].shape)
            # print("DE mse:", ((results['pred_de'][p_idx_de] - results['truth_de'][p_idx_de]) ** 2).mean())
            # p_idx_de = np.where(results['pert_cat_de'] == pert)[0]
            # for m, fct in metric2fct.items():
            #     if len(p_idx_de) == 0:
            #         val = 0
            #     elif m == 'pearson':
            #         val = fct(results['pred_de'][p_idx_de].mean(0), results['truth_de'][p_idx_de].mean(0))[0]
            #         val = 0 if np.isnan(val) else val
            #     else:
            #         val = fct(results['pred_de'][p_idx_de].mean(0), results['truth_de'][p_idx_de].mean(0))

                metrics_pert[pert][m + '_de'] = val
                metrics[m + '_de'].append(val)
