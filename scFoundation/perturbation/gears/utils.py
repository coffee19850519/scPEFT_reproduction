...
def compute_edges_for_gene(g1_gene2go_tuple):
    g1, gene2go = g1_gene2go_tuple
    edges = []
    for g2 in gene2go.keys():
        set1, set2 = set(gene2go[g1]), set(gene2go[g2])
        inter = len(set1 & set2)
        union = len(set1 | set2)
...
def get_go_auto(gene_list, data_path, data_name):
    ...
    if os.path.exists(go_path):
        ...
        with Pool(processes=cpu_count()-1) as pool:
            results = list(
                tqdm(pool.imap(compute_edges_for_gene, [(g1, gene2go) for g1 in gene2go.keys()]), total=len(gene2go)))

        # flatten
        edge_list = [edge for sublist in results for edge in sublist]
        #
...
class GeneSimNetwork():
    def __init__(self, edge_list, gene_list, node_map):
        ...
        importance = np.array([edge_attr[e] for e in self.G.edges if e[0] in node_map and e[1] in node_map])
...
def get_similarity_network(network_type, adata, threshold, k, gene_list, data_path, data_name, split, seed, train_gene_set_size, set2conditions, gi_go = False, dataset = None):
    ...
    if network_type == 'co-express':
        ...
        # df_jaccard.to_csv('/fs/ess/PAS1475/Yang/yy/scFoundation_prompt/GEARS/data/gse133344_k562gi_oe_pert227_84986_19264_withtotalcount/df_jaccard.csv', index=False)
        # if gi_go:
...
def loss_fct(pred, y, perts, ctrl = None, direction_lambda = 1e-3, dict_filter = None):
    ...
    perts = np.array(perts)
    # print(perts)
    # print(dict_filter.keys())
    losses = torch.tensor(0.0, requires_grad=True).to(pred.device)

    for p in set(perts):
        if p in dict_filter.keys() or p=='ctrl':
