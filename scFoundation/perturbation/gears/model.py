    ...
    def forward(self, data):
        ...
        if self.no_perturb:
            ...
            if self.pretrained:
                ...
                # print('pre_in:',pre_in.shape)
                # print('num_graphs:',num_graphs)
                # print('self.num_genes:', self.num_genes)
                x = x.reshape(num_graphs, self.num_genes+1)[:,:-1]
                emb = self.singlecell_model(pre_in)
                # print(emb)
                # print(self.singlecell_model)

                # print(f"[DEBUG] emb.shape before reshape: {emb.shape}")
                # emb = emb.view(num_graphs, self.num_genes + 1, self.hidden_size)
                # emb = emb[:, :-1, :].reshape(-1, self.hidden_size)
                if 'v1' in self.args['mode']:
                    # self.num_genes = emb.shape[1]
                    # print('num_tokens', num_tokens)
                    # pos_emb = self.emb_pos(torch.arange(num_tokens).repeat(num_graphs).to(self.args['device']))
                    pos_emb = self.emb_pos(torch.LongTensor(list(range(self.num_genes))).repeat(num_graphs, ).to(self.args['device']))
                    # print('v1',pos_emb.shape)
                ...
                pos_emb = self.emb_pos(torch.LongTensor(list(range(self.num_genes))).repeat(num_graphs, ).to(self.args['device']))
            # print(f"[DEBUG] emb.shape before bn_emb: {emb.shape}")
            # emb_cpu = emb.detach().cpu()
            # print(f"[DEBUG] emb has NaN: {torch.isnan(emb_cpu).any().item()}")
            # print(f"[DEBUG] emb has Inf: {torch.isinf(emb_cpu).any().item()}")
            # print(f"[DEBUG] emb min: {emb_cpu.min().item()}, max: {emb_cpu.max().item()}")
            # print(emb)
            emb = self.bn_emb(emb)
            base_emb = self.emb_trans(emb)        
            # pos_emb = emb
            for idx, layer in enumerate(self.layers_emb_pos):
                # print("edge_index shape:", self.G_coexpress.shape)
                # print("edge_weight shape:", self.G_coexpress_weight.shape)
