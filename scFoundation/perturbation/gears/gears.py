...
class GEARS:
    def __init__(self, pert_data, 
        ...
        ctrl_idx = np.where(self.adata.obs["condition"].values == "ctrl")[0]
        self.ctrl_expression = torch.tensor(np.mean(self.adata.X[ctrl_idx], axis=0)).reshape(-1, ).to(self.device)
    ...
    def train(self, epochs = 20, result_dir='./results',
        ...
        elif self.config['finetune_method'] == 'prompt':
        ...
        print("-" * 89)
        learnable_params = {k: v for k, v in self.model.named_parameters() if v.requires_grad == True}
        ...
        print("Total Params: %.2fM" % (param_count / 1e6,))
        print("-" * 89)
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                if params:  # Only clip if there are gradients
                ...
                if step % 50 == 0:
                    ...
                    # print(f"  Cached: {torch.cuda.memory_allocated(0) / 1024 ** 2:.2f} MB")
    ...
    def test(self,result_dir='./results'):
        test_loader = self.dataloader['test_loader']
