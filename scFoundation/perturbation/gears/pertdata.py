...
def main_gene_selection(adata, gene_list):
    """
    Describe:
        Rebuild the input adata to select target genes encoding protein.
    Parameters:
        adata : `~anndata.AnnData` object
            AnnData with gene symbols as `adata.var_names`.
        gene_list : list
            List of target genes to retain.
    Returns:
        adata_new : `~anndata.AnnData` object
            New AnnData containing only target genes.
        to_fill_columns : list
            List of genes in gene_list that were not originally present in adata.
        var : pd.DataFrame
            DataFrame with a binary 'mask' column indicating padded genes (1) or original genes (0).
    """
    # Create a DataFrame from adata.X using gene symbols as columns
    ...
    # Zero-padding for missing genes
    ...
    X_df = X_df[gene_list]  # Reorder columns according to gene_list

    # Create var DataFrame with mask
    var = pd.DataFrame(index=X_df.columns)
    var['mask'] = [1 if gene in to_fill_columns else 0 for gene in var.index]
...
class PertData:
    ...
    def load(self, data_name = None, 
        ...
        if data_name in ['norman', 'adamson', 'dixit']:
            ...
            gene_list_df = pd.read_csv('/fs/ess/PAS1475/Yang/yy/scFoundation_prompt/model/OS_scRNA_gene_index.19264.tsv', header=0, delimiter='\t')
            gene_list = list(gene_list_df['gene_name'])
            self.adata,_,_ = main_gene_selection(self.adata, gene_list)
            print(self.adata)
    ...
    def create_cell_graph_dataset(self, split_adata, pert_category,
        ...
        """
        sc.pp.calculate_qc_metrics(split_adata, inplace=True)
        num_de_genes = 20        
        ...
        if pert_category != 'ctrl':
            ...
            for cell_z in adata_.X:
                ...
                # print(ctrl_samples.obs.columns)
