...
def select_module(config, sub_config, module_name,use_prompt,num_tokens,prompt_type,n_layers_conf,mlp_adapter_conf,space_adapter_conf):
...
class MAEAutobinencoder(nn.Module):
    def __init__(self, config, hidden_size=None):
        ...
        self.prompt_type = self.config['prompt_type']
    def init_module(self, ckp):
        model_type = self.model_type
        model_config = self.model_config
        print(model_config)
        ...
        encoder = select_module(model_config, encoder_config, model_config['encoder']['module_type'],self.config['use_prompt']
                                ,self.config['num_tokens'],self.config['prompt_type'],self.config['n_layers_conf'],self.config['mlp_adapter_conf'],self.config['space_adapter_conf'])
        decoder = select_module(model_config, decoder_config, model_config['decoder']['module_type'],self.config['use_prompt']
                                ,self.config['num_tokens'],self.config['prompt_type'],self.config['n_layers_conf'],self.config['mlp_adapter_conf'],self.config['space_adapter_conf'])
        ...
        self.m = model
        m_state_dict = ckp["model_state_dict"]
        self.m.load_state_dict(m_state_dict, strict=False)
