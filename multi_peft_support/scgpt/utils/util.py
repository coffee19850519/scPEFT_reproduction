...
class PeftConfig:
    ...
    @classmethod
    def create_multi_peft_config(cls, peft_strategies, model_nlayers=12, use_default_settings=True):
        """
        Create a PEFT configuration that supports multiple strategies simultaneously.
        
        Args:
            peft_strategies: List of PEFT strategies to enable (e.g., ["ENCODER", "LORA", "PREFIX"])
            model_nlayers: Number of model layers
            use_default_settings: Whether to use default settings
            
        Returns:
            Dictionary containing the combined PEFT configuration
        """
        if not peft_strategies or peft_strategies == [False]:
            return {}
            
        # Create base configuration
        config = {
            "peft_type": "MULTI_PEFT",
            "model_nlayers": model_nlayers,
            "use_default_settings": use_default_settings,
            "adapter_layer_conf": [item <= int(model_nlayers / 2) for item in range(1, model_nlayers + 1)],
            "ENCODER_FLAG": "ENCODER" in peft_strategies or "HYBRID" in peft_strategies,
            "TOKEN_FLAG": "TOKEN" in peft_strategies or "HYBRID" in peft_strategies,
            "PREFIX_FLAG": "PREFIX" in peft_strategies,
            "LoRA_FLAG": "LORA" in peft_strategies,
        }
        
        # Add strategy-specific default settings
        if config["ENCODER_FLAG"]:
            config.update({
                "mlp_ratio": 0.25,
                "skip_connect": True,
            })
        
        if config["TOKEN_FLAG"]:
            config.update({
                "mlp_ratio": config.get("mlp_ratio", 1),  # Use existing or default to 1 for TOKEN
                "skip_connect": True,
            })
        
        if config["PREFIX_FLAG"]:
            config.update({
                "token_nums": 8,
            })
