...
from .utils import print_sys, zip_data_download_wrapper, filter_pert_in_go
...
class PertData:
    ...
    def load(self, data_name=None,
        ...
        if data_name in ['norman', 'adamson', 'dixit',
            ...
            # add by ruixin for filter some perturbed genes that not in feature list
            not_in_go_pert = np.array(self.adata.obs[
                                          self.adata.obs.condition.apply(
                                              lambda x: not filter_pert_in_go(x,
                                                                              self.adata.var[
                                                                                  'gene_name'].tolist()))].condition.unique())
            # Removed debug print of excluded perturbations

            filter_go = self.adata.obs[self.adata.obs.condition.apply(
                lambda x: filter_pert_in_go(x, self.adata.var['gene_name'].tolist()))]
            self.adata = self.adata[filter_go.index.values, :]

            # Get the set of condition_name values
            condition_set = set(self.adata.obs['condition_name'])
            filtered_non_zeros_gene_idx = {k: v for k, v in self.adata.uns['non_zeros_gene_idx'].items() if
                                           k in condition_set}
            self.adata.uns['non_zeros_gene_idx'] = filtered_non_zeros_gene_idx

            # Removed non-essential debug prints to reduce noise
        ...
        if os.path.isfile(dataset_fname):
            # Keep prints focused on training/model; suppress local cache notices
            self.dataset_processed = pickle.load(open(dataset_fname, "rb"))
        else:
            self.ctrl_adata = self.adata[self.adata.obs['condition'] == 'ctrl']
            self.gene_names = self.adata.var.gene_name
            # Suppress verbose creation notices
    ...
    def prepare_split(self, split='simulation',
        ...
        if os.path.exists(split_path):
            ...
            # Suppress verbose split creation notices
            if test_perts:
            ...
            # Suppress verbose save notices

        self.set2conditions = set2conditions

        if split == 'simulation':
            # Suppress detailed simulation composition printing
            pass
    ...
    def get_dataloader(self, batch_size, test_batch_size=None):
        ...
        if self.split == 'no_split':
            ...
            # Suppress verbose dataloader creation notices
            # Set up dataloaders
            ...
            if self.split != 'no_test':
                ...
                self.dataloader = {'train_loader': train_loader,
                                   'val_loader': val_loader}
            # Suppress completion notice to reduce non-essential output
