...
# Import PEFT components from model.py
from .model import (
...
class MultiOmicTransformerModel(nn.Module):
    def __init__(
        ...
        # Batch Encoder
        if use_batch_labels:
            self.batch_encoder = BatchLabelEncoder(num_batch_labels, d_model, peft_config=peft_config)

        if use_mod:
            self.mod_encoder = BatchLabelEncoder(
                ntokens_mod, d_model, padding_idx=vocab_mod[pad_token], peft_config=peft_config
            )
        ...
        if use_fast_transformer:
            ...
            if peft_config is None or all(value is False for value in peft_config.values()):
...
class ContinuousValueEncoder(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_value: int = 512, peft_config: dict = None):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
        ...
        # PEFT support for value encoder
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.linear2(x)
        x = self.norm(x)
        x = self.adapter(x)  # Apply PEFT adapter if enabled
        return self.dropout(x)
...
class BatchLabelEncoder(nn.Module):
    def __init__(
        ...
        # PEFT support for batch/modal encoders
        if peft_config is not None:
            TOKEN_FLAG = peft_config.pop("TOKEN_FLAG", False)
            self.adapter = Adapter(feature_dim=embedding_dim, **peft_config) if TOKEN_FLAG else nn.Identity()
        else:
            self.adapter = nn.Identity()
