...
def prepare_data(
    ...
    assert config.task in ["identification", "integration", "perturb", "multiomic"]
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            output_dict = model(
                ...
                CLS=config.CLS,
                MVC=config.GEPC,  # Use GEPC like original version
                ECS=getattr(config, 'ESC', getattr(config, 'ECS', False)),  # Backward compatibility
                mod_types=mod_types if config.task == "multiomic" and config.use_mod else None,
            ...
            if getattr(config, 'ESC', getattr(config, 'ECS', False)):
        ...
        total_ecs += loss_ecs.item() if getattr(config, 'ESC', getattr(config, 'ECS', False)) else 0.0
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            cur_ecs = total_ecs / log_interval if getattr(config, 'ESC', getattr(config, 'ECS', False)) else 0.0
            ...
            logger.info(
                ...
                + (f"ecs {cur_ecs:5.2f} |" if getattr(config, 'ESC', getattr(config, 'ECS', False)) else "")
                + (f"dar {cur_dab:5.2f} |" if config.DAB else "")
            )
            total_loss = 0
            total_gep = 0
            total_cls = 0
...
def evaluate(
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            if config.DAB:
                total_dab += (
                    loss_dab.item() * len(input_gene_ids) if config.DAB else 0.0
                )
            else:
                total_dab = 0
        ...
        for batch_data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=config.amp):
                output_dict = model(
                    ...
                    CLS=config.CLS,
                    MVC=config.GEPC,
                    ECS=getattr(config, 'ESC', getattr(config, 'ECS', False)),
                    mod_types=None,  # predict function doesn't use mod_types
                )
    ...
    if "cls" in include_types:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=config.amp):
            cell_embeddings = model.encode_batch(
                ...
                batch_size=config.batch_size,
                batch_labels=torch.from_numpy(batch_ids).long()
                if config.DSBN or config.DAB or config.use_batch_labels
                else None,
                time_step=0,
                return_np=True,
            )
        ...
        try:
            ...
            # Suppress verbose traceback; log concise error instead
            logger.error(e)
