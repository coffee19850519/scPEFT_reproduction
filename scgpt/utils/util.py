...
def load_pretrained(
    ...
    use_flash_attn = getattr(model, "use_fast_transformer", False)
...
def eval_scib_metrics(
    ...
    logger.info(f"avg bio (cell-type): {result_dict['avg_bio']:.4f} \n")
    logger.info(f"avg batch (batch): {result_dict['avg_batch']:.4f} \n")
    logger.info(
        f"overall(0.6*avg bio + 0.4*avg batch): {result_dict['avg_bio'] * 0.6 + result_dict['avg_batch'] * 0.4:.4f} \n")
