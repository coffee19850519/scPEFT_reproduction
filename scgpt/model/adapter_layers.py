...
class OFTLinear(nn.Linear):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.block_share:
            ...
            self.oft_R = nn.Parameter(self.weight.new_zeros(R_shape[0], R_shape[0]), requires_grad=True)
            ...
            self.oft_R = nn.Parameter(self.oft_R, requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.block_share:
            R_used = self.oft_R
            if self.is_coft:
                R_used = project(self.oft_R, eps=self.eps)
            orth_rotate = self.cayley(R_used)
        else:
            R_used = self.oft_R
            if self.is_coft:
                R_used = project_batch(self.oft_R, eps=self.eps)
            orth_rotate = self.cayley_batch(R_used)
...
class OFTInjectedLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.block_share:
        #     if self.is_coft:
        #         R_used = project(self.R, eps=self.eps).detach()
        #     else:
        #         R_used = self.R
        #     orth_rotate = self.cayley(R_used)
        # else:
        #     if self.is_coft:
        #         R_used = project_batch(self.R, eps=self.eps).detach()
        #     else:
        #         R_used = self.R
        #     orth_rotate = self.cayley_batch(R_used)
