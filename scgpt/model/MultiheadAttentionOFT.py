...
def multi_head_attention_forward(
    ...
    if not use_separate_proj_weight:
        ...
        merged_qkv = Wqkv(query)
        B, N, _ = merged_qkv.shape
        E = merged_qkv.size(-1) // 3

        # split to q, k, v
        merged_qkv = merged_qkv.view(B, N, 3, E).permute(2, 0, 1, 3).contiguous()
...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        # print('self._qkv_same_embed_dim',self._qkv_same_embed_dim,flush=True)
        if not self._qkv_same_embed_dim:
            ...
            self.Wqkv = OFTInjectedLinear(embed_dim, 3 * embed_dim,bias =True, **OFT_settings)
            # print(self.Wqkv)
            self.in_proj_weight = self.Wqkv.OFT.weight
            self.in_proj_bias = self.Wqkv.OFT.bias
